...
export default function PenunjangPage() {
  ...
  // State: data + loading
  // =========================
  const [data, setData] = useState<PenunjangItem[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.namaKegiatan || !formData.tahunAjaran || !formData.semester || !formData.peran) {
      toast.error('Nama kegiatan, tahun ajaran, semester, dan peran wajib diisi.');
      return;
    }
    ...
    try {
      ...
      if (!res.ok || json?.status !== 'success') {
        toast.error(json?.message || 'Gagal menyimpan data penunjang.');
        return;
      }

      toast.success('Data penunjang berhasil disimpan.');
      setFormData((prev) => ({
        ...prev,
        namaKegiatan: '',
        peran: '',
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold text-gray-800 border-b pb-4">Bidang Penunjang</h1>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <p className="mt-1 text-xs text-gray-500">Opsional. Jika diisi, status akan otomatis “SUDAH UPLOAD”.</p>
          </div>
          </div>
      ...
      {/* Table */}
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
        <h2 className="text-xl font-semibold text-gray-800 border-b pb-4">Riwayat Kegiatan</h2>

        {isLoading ? (
          <p className="mt-4 text-gray-600">Memuat data...</p>
        ) : data.length === 0 ? (
          <p className="mt-4 text-gray-600">Belum ada data penunjang.</p>
        ) : (
                ...
                {data.map((item) => (
                  <tr key={item.id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      ...
                      <div className="text-xs text-gray-500 mt-1">{item.tahun_semester}</div>
                    </td>

                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{item.detail_kegiatan}</td>
      ...
      {showUpload ? (
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow">
            <h3 className="text-lg font-semibold text-gray-800">Upload Bukti Penunjang</h3>
          ...
          <div className="w-full max-w-4xl rounded-lg bg-white p-4 shadow">
            ...
            <div className="mt-4">
              {previewIsPdf ? (
                ...
                <img src={previewUrl} alt="Preview Bukti" className="mx-auto max-h-[70vh] w-full rounded-md object-contain" />
              )}
            </div>
